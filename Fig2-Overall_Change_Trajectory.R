...
mat_raw <- readr::read_csv("Input/matrix_features.csv", col_types = cols()) 
...
# remove T0 for plotting panel C if you want only change after deployment
mean_pairwise_cent_no0 <- mean_pairwise_cent %>%
  filter(Time_yrs > 0)

# 8.2 Distance to grand centroid (for convergence coloring of segments)
...
fig2A_NMDS <- ggplot(
  ...
  scale_color_manual(values = my_colors, name = "Litter type") +
...
fig2B_PCoA_conv <- ggplot() +
  ...
  geom_segment(data = segments_conv,
               ...
               alpha = 0.9) +
  scale_fill_manual(values = my_colors, name = "Litter type") +
  scale_color_manual(
    values = c(
      "Converging" = "#1f78b4",
      "Diverging"  = "#e31a1c",
...
## 11. Assemble final multi-panel figure -----------
