...
### Van krevelen Pairwise Consective timepoints – ANG
### Figure 6 
...
ggsave("Plots/Combined_VK_unique_0_0.5_1_2.png",
...
gfe_labels <- presence_df %>%
  group_by(Class, Pickup_t) %>%
  summarise(
    mean_gfe = mean(GFE, na.rm = TRUE),
    sd_gfe = sd(GFE, na.rm = TRUE),
    label = sprintf("%.2f ± %.2f", mean_gfe, sd_gfe),
    .groups = "drop"
  )
...
fig6_full <- top_row / bottom_row / legend_row +
  plot_layout(heights = c(2, 1.3, 0.25)) +
...
fig6_full <- main_panels / legend_row +
  plot_layout(heights = c(2, 1.3, 0.25))  # adjust as needed

fig6_full


ggsave(
  filename = "Plots/Fig6_VK_mass_ANG_5panel.png",
  plot     = fig6_full,
  width    = 11,
  height   = 7.5,
  dpi      = 300
...
ggsave(
  filename = "Plots/SupFig6_GFE_NOSC_ANG.png",
  plot     = Sup_fig6,
  width    = 8,
  height   = 8,
  dpi      = 300
