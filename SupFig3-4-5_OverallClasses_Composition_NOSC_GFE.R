...
## Sup Fig 3-4-5 - overall shift in compound classes: 

## - Uses MetaboDirect outputs
## - Plots:
##   Sup Fig 3: Significant molecular classes over time
##   Sup Fig 4: Significant elemental composition over time
##   Sup Fig 5: NOSC (A) and GFE (B) with time (weighted)
...
# Color palette for Pickup_t
# my_colors <- set_names(
#   ggpubr::get_palette("Dark2", length(unique(metadata[[group1]]))),
#   nm = levels(metadata[[group1]])
# )
...
p_box_class_sig
ggsave("Plots/SupFig3_Significant_molecular_classes_over_time.png",
...
p_box_el_sig
ggsave(
  file.path("Plots/SupFig4_Significant_elemental_composition_over_time.png"),
  p_box_el_sig, dpi = 300, width = 10, height = 6
)


## Sup Fig 5 – NOSC (A) and GFE (B) with time ----
library(rstatix)
library(rlang)
library(ggpubr)
...
ggsave(
  file.path("Plots/SupFig5_NOSC_GFE_violin_boxplots_over_time.png"),
  sup_fig5, dpi = 300, width = 8.5, height = 4
)
