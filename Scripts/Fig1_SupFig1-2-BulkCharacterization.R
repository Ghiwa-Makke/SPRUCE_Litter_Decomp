...
# Litter orderings --------------------------------------------------------
decomp_order  <- c("MAG", "LTR", "SPR", "ANG", "LTL", "SPL")  # by k
...
df_T0 <- df %>%
  filter(Pickup_t == "T_0") %>%
  mutate(Litter = factor(Litter, levels = k2_order))
...
supp_fig1 <- ggpubr::ggarrange(
  ...
  ncol    = 1,
  nrow    = 2,
  heights = c(1.5, 1)       # give A a bit more height if you like
)

supp_fig1

ggsave("Plots/SuppFig1_tempCO2_k_plus_initial_CNP.png",
       plot   = supp_fig1,
       dpi    = 300,
       width  = 7.5,
       height = 6.5)
...
# Label mapping for time (already applied in your previous version)
time_labels <- c(
  "T_0.5" = "0.5 year",
  "T_1"   = "1 year",
  "T_2"   = "2 years"
...
df_long_arom <- df_long %>%
  filter(Metric == "percent_aromatics_change")

supp2_arom <- ggplot(df_long_arom,
                     aes(x = Litter, y = Change, fill = Litter)) +
  geom_boxplot(alpha = 0.9, outlier.size = 0.6) +
  scale_fill_manual(values = my_colors) +
  facet_wrap(~ Pickup_t, nrow = 1,
             labeller = labeller(Pickup_t = time_labels)) +
  labs(
    x = "Litter type",
    y = "Percent change in aromatics (%)"
  ) +
  theme_bw(base_size = 11) +
  theme(
    axis.text.x     = element_text(angle = 25, vjust = 1, hjust = 1),
...
ggsave("Plots/SuppFig2_percent_change_CNP_and_aromatics.png",
       plot   = supp_fig2,
       dpi    = 300,
       width  = 7,
       height = 6)
