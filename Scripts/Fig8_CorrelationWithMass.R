...
div <- read.csv("Tables/Class_Shannon_Diversity_PerSample.csv") %>% # R script output from Figure 7
  ...
  for (temp in unique(mydata$Temp)) {
    for (co2 in unique(mydata$CO2)) {
      ...
      slope_table_all <- rbind(
        slope_table_all,
        data.frame(Litter = litter_type, Temp = temp, CO2 = co2,
...
rownames(slope_table_all) <- paste(slope_table_all$Litter, slope_table_all$Temp, slope_table_all$CO2, sep = "_")
write.csv(slope_table_all, "Tables/Slope_table_all_WLS.csv", row.names = TRUE)

#  Correlations
data_matrix <- slope_table_all[, 4:13]

spearman_corr_matrix <- cor(data_matrix, method = "spearman", use = "pairwise.complete.obs")
...
p_values <- sapply(names(data_matrix), function(v) {
  ...
  suppressWarnings(cor.test(xy[[1]], xy[[2]], method = "spearman", exact = FALSE)$p.value)
})
...
# ordering for classes
compound_order <- c("Tannin", "Lignin", "Carbohydrate", "Condensed HC", "Lipid",
...
bar_plot <- ggplot(mass_corr_df,
  ...
  scale_fill_gradient2(low = "#40B0A6", mid = "white", high = "yellow4",
...
figure_8
ggsave("Plots/Figure8_MassLoss_vs_ClassCorrelations.png",
       figure_8, width = 14, height = 6, dpi = 300, bg = "white")

#  ggpairs ---------------------------
plot_pairs <- ggpairs(
  slope_table_all, columns = 4:13,
...
ggsave("Plots/ggpairs_plot.png", plot_pairs, width = 14, height = 11, dpi = 300, bg = "white")
