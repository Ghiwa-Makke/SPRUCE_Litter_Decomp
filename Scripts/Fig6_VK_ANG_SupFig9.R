...
### Van Krevelen pairwise consecutive timepoints - ANG
### Figure 6
...
## Data
intensity_df <- read_csv("Input/Report_processed_noNorm_ANG.csv")
metadata <- read_csv("Input/metadata.csv") %>%
  filter(Litter == "ANG")

df_longer <- intensity_df %>%
  pivot_longer(
    cols = metadata$SampleID,
    names_to = "SampleID",
    values_to = "Intensity"
  ) %>%
...
p_vk_0_0.5 <- ggplot(plot_df_0_0.5, aes(x = OC, y = HC)) +
  geom_point(aes(color = Pickup_t), size = 1, alpha = 0.9) +
  scale_color_manual(
    values = tp_colors,
    drop   = FALSE,
...
p_vk_0.5_1 <- ggplot(plot_df_0.5_1, aes(x = OC, y = HC)) +
  ...
  theme_bw(base_size = 10) +
  labs(
    title = "Unique compounds: 0.5 vs 1 year",
    x = "O/C",
    y = "H/C"
  ) +
  theme(
...
VK_summary <- vk_summary_plot2 +
  geom_curve(
    aes(x = 0.58, y = 1.5, xend = 0.65, yend = 1.0),
    curvature = 1.3,
    angle = 80,
    ncp = 20,
...
gfe_violin_by_class <- presence_df %>%
  ...
  facet_wrap(
    ~ Class,
    scales = "free_y",
    labeller = labeller(Class = strip_labels_class)
  ) +
  theme_bw() +
  labs(
  ...
  facet_grid(
    ...
    labeller = labeller(
      ...
      Class = c(
        ...
        "Tannin" = "Tannin",
        "Lipid" = "Lipid",
        "Condensed hydrocarbon" = "Condensed HC"
      )
    )
  ) +
  theme_bw(base_size = 10) +
...
rotated_density2_noleg <- rotated_density2 + theme(legend.position = "none", plot.title = element_blank())
...
fig6_full <- cowplot::plot_grid(
  top_row,
  bottom_row,
  legend_row,
  ncol = 1,
  rel_heights = c(1.3, 2, 0.35)
)

fig6_full

ggsave(
  filename = "Plots/Updated/Figure6.tiff",
  plot     = fig6_full,
  width    = 190,
  height   = 170,
  units    = "mm",
...
top_row_sup <- nosc_violin + gfe_violin +
  plot_layout(ncol = 2)

bottom_row_sup <- gfe_violin_by_class +
  theme(legend.position = "none", plot.title = element_blank())

Sup_fig9 <- top_row_sup / bottom_row_sup +
...
ggsave(
  filename = "Plots/Updated/SuppFig9.tiff",
  plot     = Sup_fig9,
  width    = 190,
  height   = 180,
  units    = "mm",
